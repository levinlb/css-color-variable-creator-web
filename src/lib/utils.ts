 ...
 */
export function generateModifiedCss(cssContent: string, colors: Record<string, ColorInfo>): string {
			...
			if (line.includes('gradient') && line.includes(originalColor)) {
				...
				processedLine = processedLine.replace(gradientPattern, (match) => {
					return match.replace(
						new RegExp(`(^|[^a-zA-Z0-9-])${escapeRegExp(originalColor)}([^a-zA-Z0-9]|$)`, 'g'),
						`$1${variableName}$2`
					)
				})
				...
				processedLine = processedLine.replace(colorValuePattern, `$1${variableName}$2`)
				...
				for (const prop of colorProperties) {
					if (line.includes(prop) && line.includes(originalColor)) {
						...
						processedLine = processedLine.replace(propPattern, `$1${variableName}$2`)
					}
				}
