...
import { generateModifiedCss } from '@/lib/utils'
			...
			>
				{processedFiles.length === 0 ? (
					...
					>
						<div
							{...getRootProps()}
							className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-colors duration-200 backdrop-blur-sm bg-white/50 ${
								isDragActive
								...
								{(layoutMode === 'default' || layoutMode === 'colorsExpanded') && (
									<div
										className={`flex flex-col gap-6 ${
											layoutMode === 'default' ? '' : 'order-first'
										} ${isFullscreen ? 'h-full' : ''}`}
									>
